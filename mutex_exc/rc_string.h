...
public:
    ...
    RCString& Concat(const RCString& x);
...
private:
    ...
    void  SetChar(size_t index, char c);  // set the char at the given index to 'c"
    // CharProxy m_chprox;
...
public:
    ...
    char*  GetStr();                       // Return the RCString
...
public:
    ...
    CharProxy(RCString& rc_string, size_t index); // ctor, non explicit on purpose
    ...
    operator char() const;                       // defined conversion from
                                                 // CharProxy to char
    CharProxy(const CharProxy& cp_);   // disable cctor
private:
