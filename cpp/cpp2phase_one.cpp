...
public:
	...
	: m_id(++s_count)
	{
		cout << BLU "Animal::Ctor()" << m_id << RES << endl;
	}

	virtual ~Animal()
    {
        --s_count;
        cout << RED "Animal::Dtor()"  << m_id  << RES << endl;
    }


	Animal(const Animal &other) : m_id(++s_count)
	{
		cout << BLU "Animal::CCtor() "  << m_id << RES << endl;
	}
...
public:
	Squirl()
    {
        cout << CYA "Squirl::Ctor()" << RES << endl;
    }

	Squirl(const Squirl &other ): Animal(other)
    {
        cout << CYA "Squirl::CCtor()" << RES << endl;
    }
