...
static void PrintRawData(int fd, const char *name );
...
int main(void)
{
    ...
    PrintInYellow("\nRead a file raw block data\n");
    PrintRawData(fd, "Five" );
...
static void PrintRawData(int fd, const char *name )
{
	...
	if (S_ISDIR(inode.i_mode))
    {
        ...
        /* iterate over all the blocks till file name is found */
                ...
                && (entry->inode) )
        {
            ...
            {
                /* inode containing the data is found  */
        ...
        if ( inode_num != ~(int)0 )
        {
            /* load the inode of the file into the inode struct. */
            read_inode(fd, inode_num, &group, &inode);

            int block_number = inode.i_block[0];

            lseek(fd, BLOCK_OFFSET(block_number), SEEK_SET);

            char *file_data;
            if ((file_data = malloc(inode.i_size + 1)) == NULL)
            {
                /* allocate memory for the data block */
                fprintf(stderr, "Memory error\n");
                close(fd);
            ...
            printf("File is found inode# = %d\n", inode_num);
            printf("Block number = %d\n", block_number);
            printf("Reading %d [bytes]\n", inode.i_size);
            read( fd, file_data, inode.i_size + 1);
...
{
	lseek( fd                                           ,
           BLOCK_OFFSET(group->bg_inode_table) /* Inodes table block */
